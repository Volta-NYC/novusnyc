            ...
            return (
              <li key={section.id}>
                <a
                  ...
                  className={`group flex items-center gap-3 font-body text-[0.95rem] transition-colors ${active ? (useDarkText ? "font-semibold text-n-ink" : "font-semibold text-white") : (useDarkText ? "text-n-ink/60 hover:text-n-ink" : "text-white/65 hover:text-white")}`}
                >
                  <span className={`h-2.5 w-2.5 rounded-full border transition-transform duration-200 ${active ? "scale-110 border-n-orange bg-n-orange shadow-[0_0_0_3px_rgba(246,183,141,0.18)]" : (useDarkText ? "border-n-ink/40 bg-n-ink/80 group-hover:border-n-orange/80 group-hover:bg-n-orange/85" : "border-white/65 bg-white/90 group-hover:border-n-orange/80 group-hover:bg-n-orange/85")}`} />
                  {section.label}
                </a>
              </li>
            );

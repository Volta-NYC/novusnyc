...
export default function GrantTracker({
  ...
  useEffect(
    () =>
      subscribeGrantOpportunities((next, state) => {
        setRows(next);
    ...
    <section aria-labelledby="grant-tracker-title">
      <div className="mb-4 rounded-xl border border-amber-400/25 bg-amber-400/[0.06] p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h2
              id="grant-tracker-title"
              className="mt-1 font-display text-lg font-semibold text-white"
            >
              Master grant tracker
            </h2>
            <p className="mt-1 max-w-2xl text-[12px] leading-relaxed text-white/50">
              One filterable source for grants that Novus can share with small
              businesses. Keep eligibility and deadlines here instead of
              rebuilding a spreadsheet for every cycle.
            </p>
          </div>
          {canEdit && (
      ...
      <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_180px]">
        <SearchBar
          value={query}
          onChange={setQuery}
          placeholder="Search funder, eligibility, focus area…"
        />
        <Select
          value={filter}
          onChange={(e) => setFilter(e.target.value as "All" | GrantStatus)}
        >
              ...
              >
                <span className="min-w-0">
                  ...
                  <span className="mt-0.5 block truncate text-[11px] text-white/40">
                    {item.funder || "Funder not recorded"}
                  </span>
                </span>
                <span>
                  <Badge label={item.audience} />
                </span>
                <span className="truncate text-[11px] text-white/55">
                  {item.ownerMemberId
                    ? (nameById.get(item.ownerMemberId) ?? "Unknown")
                    : "Unassigned"}
                </span>
                ...
                <span
                  className={`font-mono text-[11px] tabular-nums ${overdue ? "text-red-400" : "text-white/60"}`}
                >
      ...
      <Modal
        ...
        title={
          editing === "new" ? "Add grant opportunity" : "Edit grant opportunity"
        }
      >
          ...
          <fieldset disabled={!canEdit} className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Funder">
                <Input
                  value={draft.funder}
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, funder: e.target.value }))
                  }
                />
              </Field>
              <Field label="Who can use it?">
                <Select
                  ...
                  onChange={(e) =>
                    setDraft((v) => ({
                      ...v,
                      audience: e.target.value as GrantOpportunity["audience"],
                    }))
                  }
                  options={["Small Businesses", "Novus"]}
                />
              </Field>
              <Field label="Research owner">
                <Select
                  ...
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, ownerMemberId: e.target.value }))
                  }
                >
              ...
              <Field label="Opens">
                <Input
                  ...
                  value={draft.opensOn}
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, opensOn: e.target.value }))
                  }
                />
              </Field>
              <Field label="Deadline">
                <Input
                  ...
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, deadline: e.target.value }))
                  }
                />
              </Field>
              <Field label="Amount">
                <Input
                  ...
                  value={draft.amount}
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, amount: e.target.value }))
                  }
                />
              </Field>
              <Field label="Geography">
                <Input
                  placeholder="NYC, Queens, nationwide…"
                  value={draft.geography}
                  onChange={(e) =>
                    setDraft((v) => ({ ...v, geography: e.target.value }))
                  }
                />
              </Field>
              <Field label="Stage">
                <Select
                  ...
                  onChange={(e) =>
                    setDraft((v) => ({
                      ...v,
                      status: e.target.value as GrantStatus,
                    }))
                  }
            ...
            <Field label="Link">
              <Input
                ...
                value={draft.url}
                onChange={(e) =>
                  setDraft((v) => ({ ...v, url: e.target.value }))
                }
              />
            </Field>
            <Field label="Focus areas">
              <Input
                placeholder="Restaurants, storefronts, women-owned…"
                value={draft.focusAreas}
                onChange={(e) =>
                  setDraft((v) => ({ ...v, focusAreas: e.target.value }))
                }
              />
            </Field>
            <Field label="Eligibility">
              <TextArea
                ...
                onChange={(e) =>
                  setDraft((v) => ({ ...v, eligibility: e.target.value }))
                }
              />
            </Field>
            <Field label="Required application materials">
              <TextArea
                rows={3}
                placeholder="Tax documents, narrative, budget, proof of location…"
                value={draft.requiredMaterials}
                onChange={(e) =>
                  setDraft((v) => ({ ...v, requiredMaterials: e.target.value }))
                }
              />
            </Field>
            <Field label="Last verified">
              <Input
                ...
                onChange={(e) =>
                  setDraft((v) => ({ ...v, verifiedOn: e.target.value }))
                }
              />
            </Field>
            <Field label="Notes">
              <TextArea
                ...
                value={draft.notes}
                onChange={(e) =>
                  setDraft((v) => ({ ...v, notes: e.target.value }))
                }
              />
            </Field>
          </fieldset>
            ...
            {editing && editing !== "new" && canEdit && (
              <Btn
                ...
                onClick={async () => {
                  if (
                    !window.confirm(
                      `Remove “${editing.name}” from the tracker?`,
                    )
                  )
                    return;

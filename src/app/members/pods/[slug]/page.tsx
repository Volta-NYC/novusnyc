...
import {
  subscribePods,
  subscribePodMembers,
  subscribePodMeetings,
  subscribeTeam,
  subscribeChapters,
  subscribePodAssignments,
  createPodMeeting,
  deletePodMeeting,
  updatePod,
  updatePodMeeting,
...
export default function PodDetailPage() {
  ...
  const rememberError = (key: string, error: string | null) =>
    setLoadErrors((current) =>
      current[key] === error ? current : { ...current, [key]: error },
    );

  useEffect(
    () =>
      subscribePods((rows, state) => {
        setPods(rows);
        rememberError("pods", state.error);
      }),
    [],
  );
  useEffect(
    () =>
      subscribePodMembers((rows, state) => {
        setMembers(rows);
        rememberError("roster", state.error);
      }),
    [],
  );
  useEffect(
    () =>
      subscribePodMeetings((rows, state) => {
        setMeetings(rows);
        rememberError("meetings", state.error);
      }),
    [],
  );
  useEffect(
    () =>
      subscribePodAssignments((rows, state) => {
        setAssignments(rows);
        rememberError("assignments", state.error);
      }),
    [],
  );
  useEffect(
    () =>
      subscribeTeam((rows, state) => {
        setTeam(rows);
        rememberError("team", state.error);
      }),
    [],
  );
  useEffect(
    () =>
      subscribeChapters((rows, state) => {
        setChapters(rows);
        rememberError("chapters", state.error);
      }),
    [],
  );

  const pod = useMemo(
    () => (pods ?? []).find((p) => p.slug === slug) ?? null,
    [pods, slug],
  );
  ...
  const isLit =
    !!myId && roster.some((m) => m.memberId === myId && m.role === "lit");
  const canRun = isAdmin || isLit;

  const nameById = useMemo(
    () => new Map(team.map((t) => [t.id, t.name])),
    [team],
  );
  ...
  const lits = roster
    .filter((m) => m.role === "lit")
    .map((m) => nameById.get(m.memberId) ?? "Unknown");
  ...
  const podAssignments = assignments.filter(
    (assignment) => assignment.podId === pod.id,
  );
  const activeAssignments = podAssignments.filter(
    (assignment) => assignment.status !== "Done",
  );
  const reviewAssignments = activeAssignments.filter(
  ...
  const TABS: { key: Tab; label: string }[] = [
    {
      key: "tracker",
      label: attendanceDue.length
        ? `Team tracker · ${attendanceDue.length}`
        : "Team tracker",
    },
    {
      key: "tasks",
      label: reviewAssignments.length
        ? `Work · ${reviewAssignments.length}`
        : "Work",
    },
    { key: "schedule", label: "Schedule" },
    ...(isFinancePod ? [{ key: "grants" as Tab, label: "Grant tracker" }] : []),
    ...(isOutreachPod
      ? [{ key: "outreach" as Tab, label: "Outreach tracker" }]
      : []),
    ...(isSocialPod
      ? [{ key: "content" as Tab, label: "Content pipeline" }]
      : []),
  ...
  return (
    <MembersLayout>
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded-full border px-3 py-1 text-[11px] font-semibold ${divisionMeta.soft} ${divisionMeta.accent} ${divisionMeta.border}`}
            >
              {division}
            </span>
            {pod.calendarUrl && (
              <a
                href={pod.calendarUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex min-h-10 items-center rounded-lg border border-white/10 bg-white/8 px-4 py-2 text-sm font-medium text-white/70 transition-colors hover:bg-white/14 hover:text-white"
              >
                Open calendar ↗
              </a>
            )}
      ...
      <div
        className="mb-5 flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-[#15181F] p-1"
        role="tablist"
        aria-label={`${pod.name} sections`}
      >
        {TABS.map((t) => (
          <button
            key={t.key}
      ...
      {tab === "tracker" && (
        <div className="space-y-5">
          <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">
            ...
            <StatCard
              label="Needs review"
              value={reviewAssignments.length}
              color="text-yellow-300"
            />
            <StatCard
              label="Attendance due"
              value={attendanceDue.length}
              color={attendanceDue.length ? "text-red-400" : "text-green-400"}
            />
          </div>
          <TeamTracker
            pod={pod}
            ...
            <div id="meeting-detail" className="scroll-mt-4">
              <AttendanceGrid
                ...
                onDelete={async () => {
                  if (
                    !window.confirm(
                      `Delete the ${selected.meetsOn} meeting and its attendance?`,
                    )
                  )
                    return;
...
function PodSchedule({
  ...
  const events = [
    ...meetings.map((meeting) => ({
      ...
      kind: "Meeting",
      title: meeting.title || `${pod.name} meeting`,
      meta: `${meeting.hours}h`,
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[12px] text-white/45">
            Meetings and work deadlines in one chronological view.
          </p>
        </div>
        {pod.calendarUrl && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-[11px] font-medium text-[#F6B78D] hover:underline"
          >
            Open shared pod calendar ↗
          </a>
        )}
      </div>
      {events.length === 0 ? (
        <Empty message="Nothing is scheduled yet." />
      ) : (
          ...
          {events.map((event) => {
            ...
            return (
              <Component
                key={event.id}
                {...(event.onClick
                  ? { type: "button" as const, onClick: event.onClick }
                  : {})}
                className="grid w-full grid-cols-[96px_88px_minmax(0,1fr)] items-center gap-3 border-b border-white/7 px-4 py-3 text-left last:border-0 hover:bg-white/[0.03]"
              >
                <span className="font-mono text-[11px] tabular-nums text-white/55">
                  {event.date}
                </span>
                <Badge
                  label={event.kind === "Meeting" ? "Upcoming" : event.meta}
                />
...
function ScheduleMeetingModal({
  ...
  useEffect(() => {
    if (!open) return;
    const localStart = meeting?.startsAt
      ? (() => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const hours = Math.max(0, Number(duration) || 0);
      const ends = new Date(begins.getTime() + hours * 3_600_000);
      const value = {
        meetsOn: start.slice(0, 10),
        title: title.trim(),
        hours,
      ...
      const id = meeting
        ? (await updatePodMeeting(meeting.id, value), meeting.id)
        : await createPodMeeting(pod.id, value);
      onCreated(id);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "The meeting was not scheduled.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={meeting ? "Edit pod meeting" : "Schedule pod meeting"}
    >
      <form className="space-y-4" onSubmit={submit}>
        ...
        <Field label="Agenda and running notes">
          <TextArea
            rows={6}
            placeholder="Topics, decisions, work reviewed, and next steps…"
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
          />
        </Field>
        <p className="rounded-lg border border-sky-400/20 bg-sky-400/[0.06] px-3 py-2 text-[11px] leading-relaxed text-sky-200">
          Everyone in the pod gets the normal automated reminder. After the
          call, this meeting becomes the attendance sheet.
        </p>
        {error && (
        ...
        <div className="flex gap-2">
          <Btn type="submit" variant="primary" disabled={saving || !start}>
            {saving
              ? "Saving…"
              : meeting
                ? "Save schedule"
...
function Settings({ pod }: { pod: Pod }) {
  ...
  const field =
    "w-28 rounded-md border border-white/10 bg-[#0F1014] px-2.5 py-1.5 text-[12px] text-white/90 focus:border-[#F3E28D]/40 focus:outline-none";
  ...
  return (
    <div className="max-w-lg space-y-4">
      ...
      <Field label="Shared pod calendar">
        <Input
          type="url"
          placeholder="Public Google Calendar or scheduling link"
          value={calendarUrl}
      ...
      {(
        [
          ...
          [
            ...
            setMeetingH,
            "hours",
            "Certified only for people marked Present.",
          ],
          [
            "Default task",
            taskH,
            setTaskH,
            "hours",
            "Earned when the task is marked done.",
          ],
        ] as const
        ...
        <div key={label} className="flex items-start gap-3">
          ...
          {hint && (
            <p className="mt-6 flex-1 text-[10px] leading-relaxed text-white/25">
              {hint}
            </p>
          )}
        </div>

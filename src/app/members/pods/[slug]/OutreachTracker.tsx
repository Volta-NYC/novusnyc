...
function emptyDraft(pod: Pod): Draft {
  return {
    subjectType: defaultSubjectType(pod),
    subjectName: "",
    contactName: "",
    contactEmail: "",
    sourceUrl: "",
...
export default function OutreachTracker({
  ...
  const [editing, setEditing] = useState<PodOutreachRecord | "new" | null>(
    null,
  );
  ...
  useEffect(
    () =>
      subscribePodOutreachRecords((next, state) => {
        setRows(next);
  ...
  const visible = useMemo(() => {
    ...
    return (rows ?? [])
      ...
      .sort((a, b) => {
        ...
        return (a.followUpOn ?? "9999-12-31").localeCompare(
          b.followUpOn ?? "9999-12-31",
        );
      });
    ...
    try {
      ...
      setSaveError(
        error instanceof Error
          ? error.message
          : "The outreach record was not saved.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const openCount = (rows ?? []).filter(
    (row) => row.podId === pod.id && !row.deletedAt && row.status !== "Closed",
  ).length;
    ...
    <section aria-labelledby="outreach-title">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-[12px] leading-relaxed text-white/50">
              Keep research, contact details, responses, calls, and handoffs in
              one shared queue. {openCount} open · {dueCount} due for follow-up.
            </p>
          </div>
          {canEdit && (
      ...
      <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_190px]">
        ...
        <Select
          value={status}
          onChange={(event) =>
            setStatus(event.target.value as "All" | OutreachStatus)
          }
        >
      ...
      {loadError ? (
        ...
        <Empty
          message={
            query || status !== "All"
              ? "No outreach records match those filters."
              : "No outreach has been added yet."
          }
          action={
          ...
          {visible.map((row) => {
            ...
            return (
              <button
                ...
                className="grid w-full gap-2 border-b border-white/7 px-4 py-3 text-left last:border-0 hover:bg-white/[0.035] lg:grid-cols-[minmax(210px,1.3fr)_130px_150px_120px_140px] lg:items-center lg:gap-3"
              >
                ...
                <span
                  className={`font-mono text-[11px] tabular-nums ${due ? "font-semibold text-red-400" : "text-white/60"}`}
                >
                  {row.followUpOn || "—"}
                </span>
                <span>
      ...
      <Modal
        ...
        title={editing === "new" ? "Add outreach" : "Edit outreach"}
      >
        <form className="space-y-4" onSubmit={save}>
          <fieldset disabled={!canEdit} className="space-y-4">
              ...
              <Field label="Record type">
                <Select
                  ...
                  onChange={(event) =>
                    setDraft((value) => ({
                      ...value,
                      subjectType: event.target.value as OutreachSubjectType,
                    }))
                  }
                  options={["Business", "School", "Partner Organization"]}
                />
              </Field>
              <Field label="Organization or school" required>
                <Input
                  autoFocus
              ...
              <Field label="Last contacted">
                <Input
                  ...
                  onChange={(event) =>
                    setDraft((value) => ({
                      ...value,
                      lastContactOn: event.target.value,
                    }))
                  }
                />
              </Field>
            ...
            <Field label="Notes">
              <TextArea
                rows={4}
                placeholder="What was sent, response, call outcome, and next step…"
                value={draft.notes}
                onChange={(event) =>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {canEdit && (
              <Btn
                type="submit"
                variant="primary"
                disabled={saving || !draft.subjectName.trim()}
              >
            ...
            {editing && editing !== "new" && canEdit && (
              <Btn
                ...
                onClick={async () => {
                  if (
                    !window.confirm(
                      `Remove “${editing.subjectName}” from this tracker?`,
                    )
                  )
                    return;
                  await deletePodOutreachRecord(editing.id);
                  setEditing(null);
                }}
              >
                Remove

...
-- Anything that somehow lands on it falls back to Assigned rather than to a
-- status the UI no longer offers.
UPDATE businesses SET tech_status = 'Assigned'
 WHERE tech_status = 'Building';
...
projects AS (
  SELECT member_id,
         ...
         count(*) FILTER (WHERE tech_status IN ('Assigned','Draft Ready','With Client')) AS projects_active,

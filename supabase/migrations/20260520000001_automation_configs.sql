...
(gen_random_uuid(), 'interview_invite_reminder', 'Interview invite reminder',
 ...
 'Reminder: book your Novus interview',
 '<p>Hi {{applicantName}},</p><p>Quick reminder to book your Novus interview slot:</p><p><a href="{{bookingLink}}">{{bookingLink}}</a></p><p>Please use the same name and email you used in your application.</p><p>Best,<br>Ethan Zhang</p>',
 array['{{applicantName}}','{{bookingLink}}']),
...
(gen_random_uuid(), 'applicant_accepted', 'Applicant accepted',
 ...
 '<div style="font-family:Garamond,''EB Garamond'',serif;font-size:15px;line-height:1.7;color:#111111"><p>Hi {{firstName}},</p><p>Congratulations! You''ve been accepted to Novus NYC.</p><p>You''ll be assigned to a project within the next week. Your team, tasks, and project details are organized in the member portal — sign in at <a href="https://novusnyc.org/members" style="color:#F6B78D">novusnyc.org/members</a>.</p><p>Best,<br>Ethan Zhang<br>Novus NYC</p></div>',
...
(gen_random_uuid(), 'interview_booked', 'Interview booked (applicant)',
 ...
 'Novus interview Confirmation',
 '<p>Hi {{applicantName}},</p><p>Your Novus interview is confirmed.</p><p><strong>Time:</strong> {{interviewTime}}<br><strong>Zoom:</strong> {{zoomLink}}</p><p><a href="{{googleCalendarUrl}}">Add to Google Calendar</a><br>A calendar invite (.ics) is attached to this email.</p><p>If you need to reschedule, please do so through the booking portal at novusnyc.org/book. If you have any trouble, reply to this email and we''ll sort it out.</p><p>We look forward to speaking with you.</p><p>Best,<br>Ethan Zhang</p>',
...
(gen_random_uuid(), 'interview_rescheduled', 'Interview rescheduled (applicant)',
 ...
 'Novus interview Rescheduled',
 '<p>Hi {{applicantName}},</p><p>Your <strong>Novus interview</strong> has been rescheduled.</p><p><strong>Previous time:</strong> {{previousTime}}<br><strong>New time:</strong> {{interviewTime}}<br><strong>Zoom:</strong> {{zoomLink}}</p><p><a href="{{googleCalendarUrl}}">Open in Google Calendar</a><br>A fresh calendar invite (.ics) is attached.</p><p>If you need to reschedule again, please do so through the booking portal at novusnyc.org/book. If you have any trouble, reply to this email.</p><p>Best,<br>Ethan Zhang</p>',
...
(gen_random_uuid(), 'interviewer_booking_notify', 'Interviewer — new booking',
 'Sent to the Novus interviewer when an applicant books their slot.',
 'New Novus interview Scheduled',
...
(gen_random_uuid(), 'interviewer_reschedule_notify', 'Interviewer — reschedule',
 'Sent to the Novus interviewer when an applicant reschedules to their slot.',
 'Novus interview Rescheduled to Your Slot',
...
(gen_random_uuid(), 'invite', 'Member invite',
 ...
 'Set up your Novus NYC member portal account',
 '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#1a1a1a"><img src="https://novusnyc.org/logo.png" alt="Novus NYC logo" width="36" style="margin-bottom:24px"><h2 style="margin:0 0 8px;font-size:20px">Set up your member portal account</h2><p style="margin:0 0 24px;color:#555;font-size:15px">Hi {{firstName}}, you''ve been invited to join the Novus NYC member portal.</p><a href="{{link}}" style="display:inline-block;background:#F6B78D;color:#0d0d0d;font-weight:700;padding:12px 28px;border-radius:10px;text-decoration:none;font-size:15px">Set Up Account</a><p style="margin:24px 0 0;font-size:13px;color:#888">If you didn''t expect this email, you can safely ignore it.</p></body></html>',
...
(gen_random_uuid(), 'demotion_notice', 'Demotion notice',
 ...
 'Your Novus NYC membership status has changed',
...
(gen_random_uuid(), 'monthly_portal_reminder', 'Monthly portal reminder',
 ...
 'Monthly reminder: check the Novus portal',
 '<p>Hey {{memberName}},</p><p>Just a quick reminder to log in to the <a href="https://novusnyc.org/members">member portal</a> and check for any open assignments or updates.</p>',
 array['{{memberName}}'])
...
('interviewer_booking_notify',  'Interviewer — new booking',        'Notifies the Novus interviewer when a slot is booked.',                          'interviewer_booking_notify',  true),
('interviewer_reschedule_notify','Interviewer — reschedule',        'Notifies the Novus interviewer when a booking is rescheduled to their slot.',    'interviewer_reschedule_notify',true),

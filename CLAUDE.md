# CLAUDE.md — Novus Codebase Guide
...
- Meetings hold an optional start time and meeting link. Attendance is saved as
  explicit `Present`, `Late`, `Excused`, or `Unexcused` values. `Present` earns
  the meeting hours, `Late` earns half by default, and absences earn zero. A
  complete roster finalizes the sheet.
- The default pod screen is the **Team tracker**: roster, aggregate attendance,
  certified meeting hours, approved work, and date-specific attendance/work
  cells in one horizontally scrollable table. The Member column stays sticky.
  Do not split roster and attendance back into separate primary tabs.
- The Grants & Funding pod owns the filterable `grant_opportunities` master
  tracker. Small Business Outreach and Ambassadors use
  `pod_outreach_records`; Social Media & Branding uses `pod_content_items` for
  draft → review → approval → scheduled → posted. Members may inspect the full
  records, while only the pod's LIT or owner/admin may change them.
- Pod meetings store their agenda/running notes, and pods may link a shared
  external calendar. Generic assignments remain the source for owned tasks,
  deadlines, review, reminders, and certified task hours.
- Marketing uses peach; Finance uses yellow, matching `TRACK_META`.
